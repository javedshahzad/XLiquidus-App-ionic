...
|-------|--------|-----------|------|
| **1 — Auth** | `v2026/auth` | `POST /register`, `GET /me`, `POST /profile`, `PUT /profile`, `POST /refresh`, `POST /logout`, `POST /token/validate`, `GET /status` | JWT |
...
| **3 — Cart** | `v2026/cart` | `GET /`, `GET /{cartId}`, `POST /`, `PUT /{cartId}`, `POST /{cartId}/items`, `DELETE /{cartId}/items/{itemId}`, `POST /{cartId}/checkout/estimate`, `POST /{cartId}/checkout`, `DELETE /{cartId}` | JWT |
...
**Total**: 49 endpoints across 7 route groups covering all 5 user journey phases.
